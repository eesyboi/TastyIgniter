...
class Messages extends MX_Controller {
	...
	public function index() {
		...
		if ( !file_exists('application/views/admin/messages.php')) { //check if file exists in views folder
		...
		foreach ($results as $result) {					
			$data['messages'][] = array(
				...
				'edit'			=> $this->config->site_url('admin/messages/view/' . $result['message_id'])
			);
		}
	...
	public function view() {
		...
		if ( !file_exists('application/views/admin/messages_view.php')) { //check if file exists in views folder
			show_404(); // Whoops, show 404 error page!
		}
