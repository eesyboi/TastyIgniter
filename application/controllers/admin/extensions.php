...
class Extensions extends MX_Controller {
	...
	public function index() {
		...
		if ( !file_exists('application/views/admin/extensions.php')) {
		...
		foreach ($extensions as $key => $value) {
			if ( !file_exists('application/controllers/admin/'. $value .'_module.php')) {
				$this->Extensions_model->uninstall('module', $value);
				$this->Settings_model->deleteSettings($split[0]);	
			}
		}
		
		$files = glob('application/controllers/admin/*_module.php');
