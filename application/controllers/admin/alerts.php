...
class Alerts extends MX_Controller {
	...
	public function index() {
		...
		if ( !file_exists('application/views/admin/alerts.php')) { //check if file exists in views folder
		...
		foreach ($results as $result) {					
			$data['alerts'][] = array(
				...
				'edit'			=> $this->config->site_url('admin/alerts/view/' . $result['message_id'])
			);
		}
	...
	public function view() {
		...
		if ( !file_exists('application/views/admin/alerts_view.php')) { //check if file exists in views folder
			show_404(); // Whoops, show 404 error page!
		}
