...
class Cart_module extends MX_Controller {
	...
	public function index() {
		...
		if ( !file_exists('application/views/admin/cart_module.php')) {
		...
		if ($this->input->post('modules')) {
			...
			$modules = unserialize($this->config->item('cart_module'));
		} else {
			$modules = array();
		}
		...
		$this->load->view('admin/cart_module', $data);
	...
	public function _updateModule() {
    	...
    	if (!$this->user->hasPermissions('modify', 'admin/cart_module')) {
			...
			if ($this->Settings_model->updateSettings('cart', array('cart_module' => serialize($update)))) {
