...
class Locations extends MX_Controller {
	...
	public function edit() {
		...
		if ($result) {
			...
			//Remove Food
			if ($this->input->post('delete') && $this->_deleteLocation($location_id) === TRUE) {
	...
	public function _addLocation() {
    	...
    	if (!$this->user->hasPermissions('modify', 'admin/locations')) {
			...
			$this->form_validation->set_rules('delivery_charge', 'Delivery Charge', 'trim|required|numeric');
			$this->form_validation->set_rules('min_delivery_total', 'Min Delivery Total', 'trim|required|numeric');
