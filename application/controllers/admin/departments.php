...
class Departments extends MX_Controller {
	...
	public function index() {
		...
		if ( !file_exists('application/views/admin/departments.php')) { //check if file exists in views folder
		...
		foreach ($files as $file) {
			...
			if (!in_array($file_names, $ignore_path)) {
				$permission_paths[] = $file_names;
			}
		}
		
		$data['permissions'] = $permission_paths;
	...
	public function edit() {
		...
		if ( !file_exists('application/views/admin/departments_edit.php')) { //check if file exists in views folder
			show_404(); // Whoops, show 404 error page!
		}
		...
		if ($department_info) {
			...
			//Remove Customer
			if ($this->input->post('delete') && $this->_deleteDepartment($department_id) === TRUE) {
	...
	public function _deleteDepartment($department_id = FALSE) {
    	if (!$this->user->hasPermissions('modify', 'admin/departments')) {
			...
			if ($this->input->post('delete') === '1') {
				...
				$this->session->set_flashdata('alert', '<p class="success">Department Deleted Sucessfully!</p>');
