...
class Find_table extends MX_Controller {
	...
	public function index() {
		...
		if ( !file_exists('application/views/main/find_table.php')) { //check if file exists in views folder
		...
		if (!empty($table_details['location'])) {
			$data['location'] 	= $table_details['location'];
		} else {
			$data['location'] 	= '';
		}
		
		if (!empty($table_details['location_id'])) {
			$data['location_id'] 	= $table_details['location_id'];
		} else {
			$data['location_id'] 	= '';
		}
	...
	public function _findTable($reserve_times, $interval) {
		...
		$this->form_validation->set_rules('location_id', 'Postcode', 'trim|required|integer');
		...
		$this->form_validation->set_rules('occasion', 'Occasion', 'trim|required|integer');
  		...
  		if ($this->form_validation->run() === TRUE) {
			...
			$table_details['location_id'] 	= $this->input->post('location_id');
	...
	public function _selectTime($table_details) {
		...
		$this->form_validation->set_rules('select_time', 'Select Time', 'trim|required|callback_validate_time');
  		...
  		if ($this->form_validation->run() === TRUE) {
			...
			//$table_details = array_merge($table_details, $selected_time);
	...
	public function validate_time($str) {
 	...
 	public function validate_guest($str) {
 		...
 		$result = $this->Reservations_model->checkTableSize($this->input->post('location_id'), $str);

		if ($this->input->post('location_id') && $result === FALSE) {
