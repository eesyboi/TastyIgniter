...
class Location {
	...
	public function __construct() {
		...
		if ($this->CI->session->userdata('local_location')) { 
			$local_location = $this->CI->session->userdata('local_location');
			
			$this->CI->db->where('locations.location_id', $local_location['location_id']);
			...
			if ($query->num_rows() > 0) {
				...
				//$this->CI->session->set_userdata('local_location', $local_location);
				...
				$this->local_info 			= $local_location;
