...
class Orders_model extends CI_Model {
	...
	public function addOrder($order_details = array(), $cart_items = array()) {
		...
		if (!empty($order_details['order_time'])) {
			$this->db->set('order_time', $order_details['order_time']);
			$this->db->set('date_added', mdate($date_format, $current_date_time));
			$this->db->set('date_modified', mdate($date_format, $current_date_time));
		}
		...
		if (!empty($order_details)) {
			...
			if ($this->db->affected_rows() > 0) {
				...
				$notify = $this->_sendOrderMail($order_id, $order_details, $cart_items);
								
				if ($order_details['payment'] === 'paypal') {
