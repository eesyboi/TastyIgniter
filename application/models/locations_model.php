...
class Locations_model extends CI_Model {
	...
	public function getLocalRestaurant($lat = FALSE, $lng = FALSE) {
		...
		$this->session->unset_userdata('local_location');
		
		// Search the rows in the markers table
		$sql  = "SELECT *, ( 3959 * acos( cos( radians(?) ) * cos( radians( location_lat ) ) *";
		$sql .= "cos( radians( location_lng ) - radians(?) ) + sin( radians(?) ) * sin( radians( location_lat ) ) ) )";
		$sql .= "AS distance FROM locations WHERE location_status = 1 HAVING distance < location_radius ORDER BY distance LIMIT 0 , 20";
		
		$query = $this->db->query($sql, array($lat, $lng, $lat));
	
		if ($query->num_rows() > 0) {
			$result_row = $query->first_row('array');
				
			/*$opening_hours = array();
			$this->db->from('working_hours');
			$this->db->where('location_id', $result_row['location_id']);
			$query = $this->db->get();
			...
			if ($query->num_rows() > 0) {
				...
				foreach ($query->result_array() as $row) {
					...
					$op_time = mdate($datestring, strtotime($row['opening_time']));
					$cl_time = mdate($datestring, strtotime($row['closing_time']));

					$opening_hours[] = array('day' => $weekday, 'open' => $op_time, 'close' => $cl_time);
				}
			}

			$local_location['status'] = 'OK';
    		$local_location['opening_hours'] = $opening_hours;*/

    		$local_location 	= $result_row;
	
			$this->session->set_userdata('local_location', $local_location);
	...
	public function getAutoComplete($filter_data = array()) {
		...
		if (!empty($filter_data['lat']) && !empty($filter_data['lng'])) {
			$sql  = "SELECT *, ( 3959 * acos( cos( radians(?) ) * cos( radians( location_lat ) ) *";
			$sql .= "cos( radians( location_lng ) - radians(?) ) + sin( radians(?) ) * sin( radians( location_lat ) ) ) )";
			$sql .= "AS distance FROM locations WHERE location_status = 1 HAVING distance < location_radius ORDER BY distance LIMIT 0 , 5";
	
			$query = $this->db->query($sql, array($filter_data['lat'], $filter_data['lng'], $filter_data['lat']));
		}
